...
internal interface QlicFlowControl {
    ...
    /**
     * Writes data from registered sources into the provided output stream, respecting the given size limit.
     * @param output The output stream to write data to.
     * @param sizeLimit The maximum number of bytes to write.
     * @param timeoutMs The maximum time (in milliseconds) to wait for data before timing out.
     * @return A pair containing:
     *         - Int: The number of bytes written, or -1 if the operation timed out.
     *         - () -> Unit: A callback to be invoked once the written data has been fully sent.
     *         Returns null if the flow control system has already been shut down.
     */
    fun writeDataToStream(output: OutputStream, sizeLimit: Int, timeoutMs: Long): Pair<Int, () -> Unit>?
    ...
    class Default(
        ...
        override fun writeDataToStream(
            output: OutputStream,
            sizeLimit: Int,
            timeoutMs: Long
        ): Pair<Int, () -> Unit>? {
            ...
            while (count == 0) {
                ...
                if (!permit) return -1 to compositeCallback
