...
internal class QlicStreamManager(
    ...
    /**
     * Cache of recently closed stream IDs.
     *
     * This is used as a tombstone set to suppress late-arriving frames for streams
     * that have already been closed locally. The cache is size-bounded and evicts
     * the oldest entries first (FIFO).
     */
    @VisibleForTesting
    internal class ClosedStreamTombstones(private val maxSize: Int) {

        private val cache = LinkedHashSet<Int>(maxSize)

        @Synchronized
        fun add(streamId: QlicStreamId) {
            cache.add(streamId.globalId())
        ...
        @Synchronized
        fun contains(streamId: QlicStreamId): Boolean {
            return cache.contains(streamId.globalId())
        }
    }
    ...
    //* Recently closed stream IDs used to filter out late frames.
    private val closedStreams = ClosedStreamTombstones(256)
    ...
    private fun createStream(
        ...
        if (!admitted) {
            ...
            throw IllegalStateException("StreamManager already closed!")
        }
        return stream
    }
