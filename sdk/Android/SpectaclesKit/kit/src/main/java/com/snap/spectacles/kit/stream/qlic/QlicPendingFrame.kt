...
internal sealed interface QlicPendingFrame : Closeable {
    ...
    class StreamData(
         ...
         */
        override fun get(limit: Int): QlicFrame? {
            val capacity = QlicFrame.StreamData.calculateMaxPayloadSize(id, limit)
            val payloadSize = capacity.coerceAtMost(remaining)
            return if (capacity > 0 && payloadSize >= 0) {
                remaining -= payloadSize
                if (remaining == 0) {
                    // Mark as completed, 0 doesn't mean completed here as there could be zero-length payload
                    remaining = -1
                }
