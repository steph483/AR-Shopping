...
internal class QlicProtocolEngine(
    ...
    fun setKeepAliveTime(duration: Long) {
        log.debug { "setKeepAliveTime($duration)" }
    ...
    private fun pollOutgoingPacket(): Pair<QlicPacket.Out, () -> Unit>? {
        ...
        LazyEncryptedStream(securityManager.encryptionManager, body)
            .use { output ->
                val overhead = securityManager.encryptionManager.getEncryptionOverheadSize()
                while (true) {
                    val timeout = (keepAliveIdleTime - trafficManager.getTrafficIdleDuration()).coerceAtLeast(0)
                    val (size, callback) = flowControl.writeDataToStream(output, body.getSpace() - overhead, timeout)
                        ?: return null
                    if (size < 0) {
                    ...
                    log.debug { "Packed QlicPacket, body = $size" }
...
/**
 * An OutputStream that encrypts data lazily using the provided EncryptionManager.
 *
 * @param encryptionManager The EncryptionManager used to create the encrypted stream.
 * @param outputStream The underlying OutputStream to write encrypted data to.
 */
internal class LazyEncryptedStream(
    private val encryptionManager: EncryptionManager,
    private val outputStream: OutputStream
) : OutputStream() {

    private var encryptedStream: OutputStream? = null

    override fun write(b: Int) {
        getEncryptedStream().write(b)
    }
    ...
    override fun write(b: ByteArray, off: Int, len: Int) {
        getEncryptedStream().write(b, off, len)
    }
    ...
    private fun getEncryptedStream(): OutputStream {
        if (encryptedStream == null) {
            encryptedStream = encryptionManager.createEncryptedStream(outputStream)
        }
