    ...
    override fun processServiceRequest(request: SpectaclesRequest) {
        ...
        when (request) {
            is SpectaclesApiRequest.Call -> {
                ...
                val response = imageTransferCallResponse(body)
                request.onResponse.accept(
                    SpectaclesRequest.Responses.Ongoing(response)
                )
            }
            is SpectaclesApiRequest.Notify -> {
                val body = request.payload.params.toString(Charsets.UTF_8)
                show(imageTransferNotifyBrief(body))
            }
...
private fun imageTransferCallResponse(body: String): ByteArray {
    if (body.contains("\"op\":\"img_chunk\"")) {
        return "ok".toByteArray(Charsets.UTF_8)
    }
    return "response for: $body".toByteArray(Charsets.UTF_8)
}

private fun imageTransferNotifyBrief(body: String): String {
    return when {
        body.contains("\"op\":\"img_start\"") -> "Image transfer started"
        body.contains("\"op\":\"img_end\"") -> "Image transfer complete"
        else -> "Notify: $body"
    }
}
