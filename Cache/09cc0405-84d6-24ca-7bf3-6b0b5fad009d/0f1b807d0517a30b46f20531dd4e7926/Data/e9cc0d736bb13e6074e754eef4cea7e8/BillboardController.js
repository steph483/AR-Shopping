...
class BillboardController {
    constructor(config) {
        ...
        this.shouldUpdateThisFrame = false;
        ...
        this.originalXEnabled = false;
        this.originalYEnabled = true;
        this.originalXBuffer = 0;
        this.originalYBuffer = 0;
     ...
     */
    performBillboardRotation(deltaTime, applyImmediately) {
        ...
        const isNearVertical = Math.abs(targetToCameraVector.dot(VEC_UP)) > VERTICAL_GIMBAL_LOCK_THRESHOLD ||
        ...
        if (this.yAxisCalculator.axisEnabled && !isNearVertical) {
            const yUpVector = upVector.dot(VEC_DOWN) > BillboardRotationCalculator_1.ALMOST_ONE ? VEC_DOWN : VEC_UP;
            const yRotation = this.yAxisCalculator.getRotation(yUpVector, forwardVector, targetToCameraVector, rightVector.uniformScale(-1), deltaTime);
        ...
        if (this.zAxisCalculator.axisEnabled) {
            ...
            const zRotation = this.zAxisCalculator.getRotation(forwardVector, upVector, cameraUpVector, rightVector, deltaTime);
            // Check if this axis actually rotated
     ...
     */
    applyInterpolatedRotation(slerpAmount) {
        if (this.targetRotation === null) {
            return;
        }
        ...
        const interpolatedRotation = slerpAmount >= 1.0 ? this.targetRotation : quat.slerp(currentRotation, this.targetRotation, slerpAmount);
    ...
    findInteractableManipulation(object) {
        ...
        return (0, SceneObjectUtils_1.findComponentInSelfOrParents)(object, InteractableManipulation_1.InteractableManipulation.getTypeName());
    }
    checkIsLookingAtCamera() {
    ...
    resetRotation() {
        ...
        const yUpVector = upVector.dot(VEC_DOWN) > BillboardRotationCalculator_1.ALMOST_ONE ? VEC_DOWN : VEC_UP;
        const yRotation = this.yAxisCalculator.resetRotation(yUpVector, forwardVector, targetToCameraVector, rightVector.uniformScale(-1));
