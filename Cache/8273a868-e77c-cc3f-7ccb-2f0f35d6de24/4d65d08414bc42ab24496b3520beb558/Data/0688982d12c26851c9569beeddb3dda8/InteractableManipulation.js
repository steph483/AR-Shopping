...
let InteractableManipulation = (() => {
    ...
    var InteractableManipulation = _classThis = class extends _classSuper {
        constructor() {
             ...
             */
            this.showFilterProperties = false;
        ...
        updateStartTransform() {
            ...
            const anchorPoint = this.cachedTargetingMode === Interactor_1.TargetingMode.Direct ? interactor.endPoint : interactor.startPoint;
            // Ensure that stretch value does not affect the new hitPointToTransformValue.
            this.hitPointToTransform = this.startTransform.position.sub(anchorPoint.add(interactor.direction.uniformScale(this.offsetPosition.length + this.smoothedStretch)));
        }
        updateStartValues() {
            (0, validate_1.validate)(this.manipulateRoot);
            (0, validate_1.validate)(this.interactable);
            ...
            if (this.interactors.length === 1) {
                ...
                const isFirstDirect = this.interactors[0].activeTargetingMode === Interactor_1.TargetingMode.Direct;
                const isSecondDirect = this.interactors[1].activeTargetingMode === Interactor_1.TargetingMode.Direct;
                const hasAnyDirect = isFirstDirect || isSecondDirect;
                this.cachedTargetingMode = hasAnyDirect ? Interactor_1.TargetingMode.Direct : Interactor_1.TargetingMode.Indirect;
        ...
        getTriggeringInteractors() {
            ...
            return interactors.filter((interactor) => {
                ...
                const hasEngagedTrigger = interactor.currentTrigger !== Interactor_1.InteractorTriggerType.None &&
                    (Interactor_1.InteractorTriggerType.Hover === undefined ||
                        interactor.currentTrigger !== Interactor_1.InteractorTriggerType.Hover);
        ...
        getDualInteractorDirection(interactor1, interactor2) {
            ...
            // - Indirect mode: startPoint (ray origin)
            const isFirstDirect = interactor1.activeTargetingMode === Interactor_1.TargetingMode.Direct;
            const isSecondDirect = interactor2.activeTargetingMode === Interactor_1.TargetingMode.Direct;
         ...
         */
        extractSingleAxisRotation(rotation) {
            if (this.rotationAxis === RotationAxis.All) {
                return rotation;
            }
        ...
        singleInteractorTransform(interactor) {
            ...
            if (this.enableTranslation) {
                ...
                if (this.cachedTargetingMode === Interactor_1.TargetingMode.Direct) {
                    // For Direct mode, use endPoint (fingertip) so hand rotation moves the object
                    const anchorPoint = interactor.endPoint ?? vec3.zero();
                    newPosition = anchorPoint.add(this.canRotate() ? deltaRotation.multiplyVec3(this.hitPointToTransform) : this.hitPointToTransform);
        ...
        dualInteractorsTransform(interactors) {
            ...
            (0, validate_1.validate)(this.manipulateRoot);
            (0, validate_1.validate)(this.interactable);
            const hasAnyDirect = this.cachedTargetingMode === Interactor_1.TargetingMode.Direct;
            // Each interactor uses its own appropriate point:
            // - Direct mode: endPoint (fingertip) so hand rotation moves the object
            // - Indirect mode: startPoint (ray origin)
            const isFirstDirect = interactors[0].activeTargetingMode === Interactor_1.TargetingMode.Direct;
            const isSecondDirect = interactors[1].activeTargetingMode === Interactor_1.TargetingMode.Direct;
