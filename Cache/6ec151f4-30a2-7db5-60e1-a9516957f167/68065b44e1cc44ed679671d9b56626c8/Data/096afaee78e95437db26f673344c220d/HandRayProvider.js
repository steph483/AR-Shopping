...
class HandRayProvider {
    constructor(config) {
        ...
        this.releaseStartTime = 0; // When release phase started (seconds)
        this.hasReachedStableThreshold = false; // Must reach stable before release is allowed
     ...
     */
    calculateClosingVelocity(currentDistance) {
     ...
     */
    getStableLocus(dynamicLocus, palmCenter, pinchStrength, fingerDistance, indexLocalY, plane) {
        ...
        if (this.cachedLocalOffset !== null) {
            ...
            if (!this.hasReachedStableThreshold &&
            ...
            const blendedLocus = (0, mathUtils_1.interpolateVec3)(dynamicLocus, cachedWorldPos, blendFactor);
    ...
    getRaycastInfo() {
        ...
        const useFarFieldRay = effectiveMode === HandInteractor_1.FieldTargetingMode.FarField ||
        ...
        const indexTip = indexTipRaw.add(indexTipUp.uniformScale(FingertipConstants_1.FINGERTIP_UP_OFFSET));
        ...
        const isInDirectMode = effectiveMode === HandInteractor_1.FieldTargetingMode.Direct;
        const isTransitioningToFromDirect = transitionInfo.isTransitioning &&
            (transitionInfo.toMode === HandInteractor_1.FieldTargetingMode.Direct || transitionInfo.fromMode === HandInteractor_1.FieldTargetingMode.Direct);
        ...
        const { locus: stablePinchLocus, cachedWorldPos, blendFactor } = this.getStableLocus(dynamicLocus, palmCenter, pinchStrength, fingerDistance, planeProjection.localY, effectivePlane);
        ...
        let finalLocus;
        {
            ...
            finalLocus = (0, mathUtils_1.interpolateVec3)(stablePinchLocus, indexTipWithOffset, this.innerLerpValue);
        }
        // Direction: points toward plane (opposite of normal)
        const lerpDirection = planeNormal.uniformScale(-1);
        // Debug visualization
        if (this._drawDebug) {
            this.drawDebugVisualization(finalLocus, dynamicLocus, cachedWorldPos, palmCenter, indexTip, thumbTip, lerpDirection, pinchStrength, blendFactor);
        }
        return {
     ...
     */
    drawDebugVisualization(finalLocus, dynamicLocus, cachedLocus, palmCenter, indexTip, thumbTip, direction, pinchStrength, blendFactor) {
        const crossSize = 0.5;
...
HandRayProvider.RELEASE_DURATION_SEC = 0.3;
