...
exports.PhysicalInteractionProvider = void 0;
...
const FingertipConstants_1 = require("./FingertipConstants");
 ...
 */
class PhysicalInteractionProvider extends TargetProvider_1.default {
    constructor(interactor, config) {
        super();
        this.interactor = interactor;
        this.config = config;
        this.targetingMode = Interactor_1.TargetingMode.Poke | Interactor_1.TargetingMode.Direct;
        // Hand tracking
        ...
        this.indexFingerTouchedInteractables = new Set();
        this.raycastHitMap = new Map(); // Store actual raycast hits
        this.spherecastCallbackId = 0; // For async safety monitoring
        this._pokeIsValid = true;
        this._pokeDepth = 0;
        this._tipToKnuckleDistance = 0;
        this._hasPushedThrough = false;
        this.initialPokePosition = null; // Where the surface was hit when poke started
        this.initialFingerTipPosition = null; // Where the fingertip was when poke started
        this.activePokeInteractable = null;
        this.invalidPokeInteractables = new Set();
        this.proximityColliders = [];
        this.nearbyInteractables = new Set();
        // InteractionPlane tracking (for HandInteractor's field targeting mode logic)
        this._currentInteractionPlanes = new Set();
        // Unified state
        this._currentTrigger = Interactor_1.InteractorTriggerType.None;
        this.recentlyEndedInteractable = null;
        this.previousActiveGestureType = Interactor_1.InteractorTriggerType.None; // Track what gesture just ended
        // Reusable Set for currentInteractableSet (avoids allocation each call)
        this._currentInteractableSetCache = new Set();
        this.log = new NativeLogger_1.default(`PhysicalInteractionProvider${this.config.handType}`);
    ...
    get interactionStrength() {
        if (this._currentTrigger === Interactor_1.InteractorTriggerType.Poke) {
            return this.calculatePokeInteractionStrength();
        }
        else if (this._currentTrigger === Interactor_1.InteractorTriggerType.Pinch) {
        ...
        if (currentInteractable !== null && currentInteractable !== undefined) {
            const supportsDirect = (currentInteractable.targetingMode & Interactor_1.TargetingMode.Direct) !== 0;
            if (supportsDirect) {
                return this.hand?.getPinchStrength() ?? null;
            }
        }
     ...
     */
    calculatePokeInteractionStrength() {
        const currentInteractable = this._currentInteractableHitInfo?.interactable;
    ...
    reset() {
        ...
        this._currentTrigger = Interactor_1.InteractorTriggerType.None;
        this.previousActiveGestureType = Interactor_1.InteractorTriggerType.None;
        this.recentlyEndedInteractable = null;
        this._pokeIsValid = true;
        this._pokeDepth = 0;
        this._tipToKnuckleDistance = 0;
    ...
    updateIndexFingerDetection() {
        ...
        if (!this.indexFingerProximitySensor) {
            this.indexFingerProximitySensor = this.hand.getProximitySensor(LandmarkNames_1.INDEX_TIP);
        }
        ...
        if (overlappingColliders.length === 0) {
            ...
            this.log.d(`No Interactables in proximity to index finger (callbackId=${currentCallbackId}) - skipping spherecast`);
        ...
        const radius = this._currentTrigger === Interactor_1.InteractorTriggerType.Poke ? FingertipConstants_1.FINGERTIP_MAINTAIN_RADIUS : FingertipConstants_1.FINGERTIP_SPHERECAST_RADIUS;
        this.probe.sphereCastAll(radius, startPoint, endPoint, (hits) => {
            ...
            for (const invalidInteractable of Array.from(this.invalidPokeInteractables)) {
                if (!this.indexFingerTouchedInteractables.has(invalidInteractable)) {
                    ...
                    this.log.d(`Removed ${invalidInteractable.sceneObject.name} from invalidPokeInteractables (no longer touching)`);
                }
            }
    ...
    updatePokeValidation(hits) {
        ...
        for (const hit of hits) {
            ...
            if (candidate !== null && (candidate.targetingMode & Interactor_1.TargetingMode.Poke) !== 0) {
                closestHit = hit;
    ...
    getPokeStartPoint() {
        ...
        const upOffset = isFullMesh ? 0 : FingertipConstants_1.FINGERTIP_UP_OFFSET;
        ...
        const isActivelyPoking = this._currentTrigger === Interactor_1.InteractorTriggerType.Poke;
    ...
    getPokeEndPoint() {
        ...
        const isFullMesh = handVisuals?.meshType === HandVisual_1.HandMeshType.Full;
        const upOffset = isFullMesh ? 0 : FingertipConstants_1.FINGERTIP_UP_OFFSET;
        const forwardOffset = isFullMesh ? 0 : FingertipConstants_1.FINGERTIP_FORWARD_OFFSET;
        const indexTipPos = this.hand.indexTip.position;
        const indexTipUp = this.hand.indexTip.up;
        const indexTipBack = this.hand.indexTip.position.sub(this.startPoint).normalize();
        return indexTipPos
            .add(indexTipUp.uniformScale(upOffset))
            .sub(indexTipBack.uniformScale(FingertipConstants_1.FINGERTIP_SPHERECAST_RADIUS - forwardOffset));
    }
    /**
    ...
    isValidPokeDirection(interactable) {
        ...
        // Check Y directions
        if (((interactable.acceptableYDirections & PokeYDirection.Up) !== 0 &&
        ...
        // Check Z directions
        if (((interactable.acceptableZDirections & PokeZDirection.Forward) !== 0 &&
    ...
    setupHandNearbyColliders() {
        ...
        const enterCollider = this.createCollider(this.ownerSceneObject, this.config.colliderEnterRadius, this.onProximityEnter.bind(this), null);
        this.proximityColliders.push(enterCollider);
        // Create exit collider (larger radius)
        const exitCollider = this.createCollider(this.ownerSceneObject, this.config.colliderExitRadius, null, this.onProximityExit.bind(this));
     ...
     */
    createCollider(sceneObject, radius, onOverlapEnter, onOverlapExit) {
     ...
     */
    getInteractableFromCollider(collider) {
        if (!collider || !collider.getSceneObject()) {
            return null;
        }
        // Use InteractionManager's collider-to-Interactable mapping which handles
    ...
    addInteractionPlaneFromOverlap(overlap) {
     ...
     */
    createInteractableHitInfoFromRaycast(interactable, hit, targetMode) {
     ...
     */
    createInteractableHitInfo(interactable, targetMode) {
        const position = this.getHandNearbyPosition();
        const interactableTransform = interactable.sceneObject.getTransform();
        // Create a synthetic RayCastHit
        const syntheticHit = {
            collider: interactable.sceneObject.getComponent("Physics.ColliderComponent"),
    ...
    updateInteractionState() {
        ...
        this.log.d(`State: trigger=${this._currentTrigger}, pinching=${isPinching}, ` +
        ...
        if (this._currentTrigger === Interactor_1.InteractorTriggerType.None && this._currentInteractableHitInfo !== null) {
            this.maintainHoverAfterInteractionEnd(isPinching);
            return;
        }
        // CASE 1: No active interaction - looking for new target
        if (this._currentTrigger === Interactor_1.InteractorTriggerType.None) {
        ...
        if (this._currentTrigger === Interactor_1.InteractorTriggerType.Pinch) {
     ...
     */
    maintainHoverAfterInteractionEnd(isPinching) {
        ...
        const supportsDirect = (hoveredInteractable.targetingMode & Interactor_1.TargetingMode.Direct) !== 0;
        ...
        if (shouldClearHover) {
            this._currentInteractableHitInfo = null;
            this.previousActiveGestureType = Interactor_1.InteractorTriggerType.None; // Reset
            return;
        }
        ...
        if (isPinching && stillNearby && indexStillTouching) {
            ...
            if (supportsDirect && hoveredInteractable !== this.recentlyEndedInteractable) {
                ...
                this._currentTrigger = Interactor_1.InteractorTriggerType.Pinch;
                this.recentlyEndedInteractable = null;
     ...
     */
    checkForNewInteraction(isPinching) {
        ...
        const supportsPoke = (bestCandidate.targetingMode & Interactor_1.TargetingMode.Poke) !== 0;
        const supportsDirect = (bestCandidate.targetingMode & Interactor_1.TargetingMode.Direct) !== 0;
        ...
        if (supportsPoke) {
            ...
            if (!isValidDirection || this.invalidPokeInteractables.has(bestCandidate)) {
                ...
                this._currentTrigger = Interactor_1.InteractorTriggerType.Poke;
                // Use actual raycast hit for accurate hit position/normal
                const raycastHit = this.raycastHitMap.get(bestCandidate);
                if (raycastHit) {
                    this._currentInteractableHitInfo = this.createInteractableHitInfoFromRaycast(bestCandidate, raycastHit, Interactor_1.TargetingMode.Poke);
                }
                else {
                    // Fallback to synthetic (shouldn't happen for poke, but defensive)
                    this._currentInteractableHitInfo = this.createInteractableHitInfo(bestCandidate, Interactor_1.TargetingMode.Poke);
                }
        ...
        if (supportsDirect && isPinching) {
            ...
            this._currentTrigger = Interactor_1.InteractorTriggerType.Pinch;
            this._currentInteractableHitInfo = this.createInteractableHitInfo(bestCandidate, Interactor_1.TargetingMode.Direct);
     ...
     */
    validatePokeInteraction(isPinching) {
        const currentInteractable = this._currentInteractableHitInfo?.interactable;
     ...
     */
    validatePinchInteraction(isPinching) {
        const currentInteractable = this._currentInteractableHitInfo?.interactable;
        if (!currentInteractable) {
            this._currentTrigger = Interactor_1.InteractorTriggerType.None;
     ...
     */
    clearRecentlyEndedStateIfNeeded() {
        ...
        if (this.hand.isPinching() && this._currentTrigger === Interactor_1.InteractorTriggerType.None) {
     ...
     * @inheritdoc
     */
    get currentInteractableSet() {
        this._currentInteractableSetCache.clear();
        for (const interactable of this.indexFingerTouchedInteractables) {
            const supportsPoke = (interactable.targetingMode & Interactor_1.TargetingMode.Poke) !== 0;
            const supportsDirect = (interactable.targetingMode & Interactor_1.TargetingMode.Direct) !== 0;
            if (supportsPoke) {
        ...
        if (this._currentTrigger === Interactor_1.InteractorTriggerType.None &&
            this.previousActiveGestureType === Interactor_1.InteractorTriggerType.Poke) {
            const currentInteractable = this._currentInteractableHitInfo?.interactable;
            if (currentInteractable) {
...
exports.PhysicalInteractionProvider = PhysicalInteractionProvider;
//# sourceMappingURL=PhysicalInteractionProvider.js.map